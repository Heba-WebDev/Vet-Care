...
describe('Vet logging in', () => {
    ...
    it('should successfully login a vet member', async() => {
        ...
        expect(prismaMock.veterinarians.findFirst).toHaveBeenCalledWith({ where: { email: vetRegisterDtoMock.email } });
    ...
    it('should throw an error if the vet member is unverified', async() => {
        ...
        await expect(vetsDatasource.login( vetLoginDtoMock ))
        .rejects
        .toThrow(CustomError.badRequest('Account has to be verified'))
    });
    ...
    await expect(vetsDatasource.login({ email: vetEntityVerifiedMock.email, password: 'fake-password' }))
        .rejects
        .toThrow(CustomError.badRequest('Invalid credentials'))
    });
});
