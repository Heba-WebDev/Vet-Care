...
const sendBillSchema = joi.object({
...
const sendBillValidation = async (req: Request, res: Response, next: NextFunction) => {

  try {
    await sendBillSchema.validateAsync(req.body);
    next();
  } catch (error ) {
