...
describe('Staff registration', () => {
  ...
  it('should throw an error if email already exists', async () => {
    ...
    await expect(staffDatasource.register(staffRegisterDtoMock)).rejects.toThrow(
      CustomError.badRequest('Provide a different email'),
    );
  });

  it('should throw an error if phone number already exists', async () => {
    prismaMock.staff.findFirst?.mockResolvedValueOnce(null).mockResolvedValueOnce(staffEntityMock);
    await expect(staffDatasource.register(staffRegisterDtoMock)).rejects.toThrow(
  ...
  it('should throw an error if job title does not exist', async () => {
    ...
    await expect(staffDatasource.register(dtoMock)).rejects.toThrow(
      CustomError.badRequest('Provide a valid job title [Receptionist, HR, or Manager]'),
    );
  });
});
