...
export class OwnersDatasourceImpl implements OwnersDatasource {
    ...
    async register(ownersDto: RegisterOwnerDto): Promise<OwnerEntity | null> {
        ...
        try {
            const emailExists = await this._prisma.owners.findFirst({
                where: { email },
            });
            if (emailExists)
                throw CustomError.badRequest('Email already exists');
            const numberExists = await this._prisma.owners.findFirst({
                where: { phone_number },
            });
            if (numberExists)
    ...
    async update(ownerDto: UpdateOwnerDto): Promise<OwnerEntity | null> {
        ...
        try {
            const owner = await this._prisma.owners.findFirst({
                where: { id },
            });
