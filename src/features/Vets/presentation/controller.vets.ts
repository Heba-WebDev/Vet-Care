...
export class VetsController extends BaseController {
  constructor(private readonly vetsRepo: VetsRepository) {
    super();
  }
  ...
  register = (req: Request, res: Response) => {
    ...
    new RegisterVets(this.vetsRepo)
      .execute(vetsDto!)
  ...
  getAllFormer = (req: Request, res: Response) => {
    const [error, vetsDto] = GetAllVetsDto.get(req.query);
    if (error) return res.status(400).send({ error });
    new GetAllFormerVets(this.vetsRepo)
      .execute(vetsDto!)
      .then((data) => res.json(data))
      .catch((err) => this.handleError(err, res));
  };
