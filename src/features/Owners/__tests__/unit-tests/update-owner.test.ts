...
describe('Update an owner', () => {
  ...
  it('should update an owner successfully', async () => {
    ...
    OwnerMapper.ownerEntityFromObject({
      name: updatedOwnerMock.name,
      email: updatedOwnerMock.email,
      phone_number: updatedOwnerMock.phone_number,
    });
    const result = await ownersDatasource.update({
      id: ownerMock.id,
      email: updatedOwnerMock.email,
    });
  ...
  it('should throw an error if another owner has the same email', async () => {
    ...
    await expect(
      ownersDatasource.update({ id: ownerMock.id, email: secondOwnerMock.email }),
  ...
  it('should throw an error if another owner has the same phone number', async () => {
    ...
    prismaMock.owners.findFirst?.mockReturnValueOnce(secondOwnerMock);
    await expect(
      ownersDatasource.update({ id: ownerMock.id, phone_number: secondOwnerMock.phone_number }),
