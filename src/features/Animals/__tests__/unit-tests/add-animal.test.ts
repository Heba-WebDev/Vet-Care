...
describe('Add an animal', () => {
  ...
  it('should throw an error for invalid animal type', async () => {
    await expect(
      animalsDatasource.add({ type: 'invalid-type', isSupported: true }),
    ).rejects.toThrow(CustomError.badRequest("Animal type doesn't exist"));
  });

  it('should throw an error if animal type already exists', async () => {
    prismaMock.animals.findFirst?.mockReturnValueOnce(Animal.Bird);
    await expect(animalsDatasource.add({ type: Animal.Bird, isSupported: true })).rejects.toThrow(
      CustomError.badRequest('Animal type already exists'),
    );
  });
});
