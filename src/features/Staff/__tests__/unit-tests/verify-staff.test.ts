...
describe('Staff verification', () => {
    ...
    it('should verify a staff member', async () => {
        ...
        const result = await staffDatasource.verify({
            email: staffLoginDtoMock.email,
        });
        expect(typeof result).toEqual('object');
        expect(prismaMock.staff.findFirst).toBeCalledWith({
            where: { email: staffMock.email },
        });
    ...
    it('should throw an error if the staff member is already verified', async () => {
        ...
        await expect(
            staffDatasource.verify({ email: staffEntityMock.email }),
        ).rejects.toThrow(
            CustomError.badRequest('Staff member already verified'),
        );
    });
});
